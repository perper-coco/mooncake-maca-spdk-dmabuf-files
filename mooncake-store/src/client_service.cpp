...
bool IsNofBoundaryLogEnabled() {
    static const bool enabled = []() {
        const char* value = std::getenv("MC_NOF_DEBUG");
        if (value == nullptr || value[0] == '\0') {
            return false;
        }
...
ReplicateConfig ApplyStoreReplicaOverrideFromEnv(const ReplicateConfig& config) {
    ...
    const bool has_env_override =
        std::getenv("MC_STORE_REPLICA_NUM") != nullptr ||
        std::getenv("MC_STORE_NOF_REPLICA_NUM") != nullptr;

    if (config.replica_num != 1 || config.nof_replica_num != 0) {
        if (has_env_override && IsNofBoundaryLogEnabled()) {
            LOG(INFO) << "nof_boundary replica_env_override_ignored"
    ...
    if (IsNofBoundaryLogEnabled()) {
        LOG(INFO) << "nof_boundary replica_env_override_applied"
                  << " original_replica_num=" << config.replica_num
                  << " original_nof_replica_num=" << config.nof_replica_num
                  << " resolved_replica_num=" << resolved.replica_num
                  << " resolved_nof_replica_num=" << resolved.nof_replica_num;
    }
    return resolved;
}
#endif
...
tl::expected<void, ErrorCode> Client::Put(const ObjectKey& key,
    ...
    if (IsNofBoundaryLogEnabled()) {
        ...
        LOG(INFO) << "nof_boundary client_put_begin"
                  << " key=" << key
                  << " slice_count=" << slices.size()
                  << " total_bytes=" << total_bytes
                  << " replica_num=" << client_cfg.replica_num
                  << " nof_replica_num=" << client_cfg.nof_replica_num;
    }
#endif
    if (protocol_ == "cxl") {
...
#ifdef USE_NOF
    if (IsNofBoundaryLogEnabled()) {
        LOG(INFO) << "nof_boundary client_put_master_result"
                  << " key=" << key
                  << " allocated_memory="
                  << transfer_summary.allocated_memory_replicas
                  << " allocated_nof="
                  << transfer_summary.allocated_nof_replicas
                  << " replica_count=" << start_result.value().size();
    }
#endif
...
void Client::StartBatchPut(std::vector<PutOperation>& ops,
...
#ifdef USE_NOF
    if (IsNofBoundaryLogEnabled()) {
        LOG(INFO) << "nof_boundary client_batch_put_start_request"
                  << " key_count=" << keys.size()
                  << " total_bytes=" << total_bytes
                  << " replica_num=" << config.replica_num
                  << " nof_replica_num=" << config.nof_replica_num
                  << " first_key=" << (keys.empty() ? "" : keys.front());
    }
#endif
    ...
    size_t master_success = 0;
    size_t master_failed = 0;
    size_t allocated_memory = 0;
    size_t allocated_nof = 0;
    for (size_t i = 0; i < ops.size(); ++i) {
        ...
        if (!start_responses[i]) {
            ...
            allocated_memory +=
                ops[i].transfer_summary.allocated_memory_replicas;
            allocated_nof += ops[i].transfer_summary.allocated_nof_replicas;
...
#ifdef USE_NOF
    if (IsNofBoundaryLogEnabled()) {
        LOG(INFO) << "nof_boundary client_batch_put_start_result"
                  << " key_count=" << ops.size()
                  << " master_success=" << master_success
                  << " master_failed=" << master_failed
                  << " allocated_memory=" << allocated_memory
                  << " allocated_nof=" << allocated_nof
                  << " requested_memory_per_key=" << config.replica_num
                  << " requested_nof_per_key=" << config.nof_replica_num;
    }
#endif
}
        ...
        for (size_t replica_idx = 0; replica_idx < op.replicas.size();
            ...
            if (replica.is_memory_replica() || replica.is_nof_replica()) {
                ...
                if (replica_type == ReplicaType::NOF_SSD &&
                    IsNofBoundaryLogEnabled()) {
                    const auto& nof_desc = replica.get_nof_descriptor();
                    const auto& handle = nof_desc.buffer_descriptor;
                    LOG(INFO) << "nof_boundary client_transfer_submit_nof"
                              << " key=" << op.key
                              << " replica_idx=" << replica_idx
                              << " endpoint=" << handle.transport_endpoint_
                              << " lba_offset=" << handle.buffer_address_
                              << " size=" << handle.size_
                              << " slice_count=" << op.slices.size();
                }
#endif
...
#ifdef USE_NOF
            if (pending_transfer.replica_type == ReplicaType::NOF_SSD &&
                IsNofBoundaryLogEnabled()) {
                LOG(INFO) << "nof_boundary client_transfer_complete_nof"
                          << " key=" << op.key
...
void Client::FinalizeBatchPut(std::vector<PutOperation>& ops) {
    ...
    if (IsNofBoundaryLogEnabled()) {
        ...
        for (const auto& op : ops) {
            ...
            nof_success += op.transfer_summary.successful_nof_transfers;
            nof_failed += op.transfer_summary.failed_nof_transfers;
        }
        LOG(INFO) << "nof_boundary client_batch_put_finalize_result"
                  << " key_count=" << ops.size()
                  << " success=" << success_count
                  << " failed=" << failed_count
                  << " nof_transfer_success=" << nof_success
                  << " nof_transfer_failed=" << nof_failed;
    }
#endif
}
